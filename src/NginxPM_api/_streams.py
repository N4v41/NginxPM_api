...
def streams(self, action=None, **kwargs):
    """
    ...
    _stream_url = self.url + "/api/nginx/streams"
    ...
    def stream_list():
        ...
        List all streams
        """
        return self.session.get(_stream_url).json()
    
    def stream_get(id):
        """
        Get an stream by id
        """
        return self.session.get(_stream_url + str(id)).json()
    
    def stream_delete(id):
        """
        Delete an stream by id
        """
        return self.session.delete(_stream_url + str(id)).json()
    
    def stream_disable(id):
        """
        Disable an stream by id
        """
        return self.session.post(_stream_url + str(id) + "/disable").json()
    
    def stream_enable(id):
        """
        Enable an stream by id
        """
        return self.session.post(_stream_url + str(id) + "/enable").json()
    ...
    def stream_create(inc_port, fwd_host, fwd_port, tcp_fwd, udp_fwd):
        ...
        return self.session.post(_stream_url, data=_body).json()
    ...
    def stream_update(id, inc_port, fwd_host, fwd_port, tcp_fwd, udp_fwd):
        ...
        return self.session.put(_stream_url + str(id), data=_body).json()
